...
search_criteria = (2017, 1.6, 36000)  # year, engine_volume, price
sear_cr = search_criteria
filtered_cars = {}

for brand, props in car_data.items():
    year, engine, price = props[1], props[2], props[4]
    if year >= sear_cr[0] and engine >= sear_cr[1] and price <= sear_cr[2]:
        filtered_cars[brand] = props

sorted_cars = sorted(filtered_cars.items(), key=lambda cost: cost[1][4])

for brand, props in sorted_cars[:5]:
    print(f'{brand} {props}')
